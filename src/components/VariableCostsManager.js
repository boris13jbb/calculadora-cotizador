...
const VariableCostsManager = ({ variableCosts, setVariableCosts }) => {
  ...
  const totalVariableCosts = Object.values(variableCosts).reduce((sum, item) => sum + item.cost, 0);
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6 mb-6">
      ...
      <form onSubmit={handleAddItem} className="mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="md:col-span-2">
            <label className="block text-sm text-gray-600 mb-1">Nombre del Costo *</label>
            <input
              type="text"
              name="name"
              value={newItem.name}
              ...
              placeholder="Ej: Transporte al cliente"
            />
      ...
      {Object.keys(variableCosts).length > 0 && (
        <div>
          <h4 className="font-medium mb-3">Costos Variables Registrados</h4>
          <div className="space-y-3">
            {Object.entries(variableCosts).map(([id, item]) => (
          ...
          <div className="mt-4 p-3 bg-gray-100 rounded-lg">
            <p className="text-sm font-medium">
              Total de costos variables: ${totalVariableCosts.toFixed(2)}
            </p>
          </div>
        </div>
      )}
